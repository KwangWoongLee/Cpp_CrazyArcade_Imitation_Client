...
{
	if (mServerSession == nullptr) return;
	if (mServerSession->mPlayer == nullptr) return;

	auto player = mServerSession->mPlayer;
	auto src = player->GetPosition();
	...
	player->MoveNextPosition(action);


	if (player->mDirtyFlag == true)
	{
