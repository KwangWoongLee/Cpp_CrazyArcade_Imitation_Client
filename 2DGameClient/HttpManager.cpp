...
uint64 HttpManager::aidx = 320;
string HttpManager::session;
string HttpManager::name = "kwlee";
string HttpManager::nickname = "woongda";
...
std::tuple<string, uint16, uint64> HttpManager::EnterRoom(Json::Value& userInfo, uint64 roomId)
{
	...
	cout << "접속정보 : " << userInfo["aidx"].asUInt64() << "가 " << host << ":" << port << " 서버 의 방 \"" << tcpRoomId << "\" 으로 접속합니다." << endl;
...
void HttpManager::GetGameResult(uint64 roomId)
{
	...
	cout << "없앤 블록은 총 "<< kill_score <<" 개입니다. " << endl;
}
